  ...
  $.dlxQuiz = function ( element, options ) {
    ...
    plugin.method = {
	  ...
	  hasil = totalScore.toFixed(0);
        ...
        if ( hasil == 100 ) {
          //top rank
          resultsHTML += hasil+" - "+plugin.config.quizScoreRank_text.a;
        } else if ( hasil > 80 ) {
          //rank 1: 80-100
          resultsHTML += hasil+" - "+plugin.config.quizScoreRank_text.g;
        } else if ( hasil > 60 ) {
          //rank 2: 60-79
          resultsHTML += hasil+" - "+plugin.config.quizScoreRank_text.b;
        } else if ( hasil > 40 ) {
          //rank 3: 40-59
          resultsHTML += hasil+" - "+plugin.config.quizScoreRank_text.c;
        } else if ( hasil > 20 ) {
          //rank 4: 20-39
          resultsHTML += hasil+" - "+plugin.config.quizScoreRank_text.d;
        } else {
          //rank 5: 0-19
          resultsHTML += hasil+" - "+plugin.config.quizScoreRank_text.f;
        }
        resultsHTML += '</h1>';
        ...
        resultsHTML += plugin.config.quizScore_text
          .replace( '%hasil', correctAnswerCount )
          .replace( '%totalQuestions', questionCount );
        resultsHTML += '</p>';
        //quiz score message
